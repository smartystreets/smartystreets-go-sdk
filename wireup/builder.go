...
func (b *clientBuilder) withHeaderCredential(authId, authToken string) *clientBuilder {
	b.credential = sdk.NewHeaderCredential(authId, authToken)
	return b
}
...
func (b *clientBuilder) buildHTTPClient() (wrapped internal.HTTPClient) {
	...
	wrapped = internal.NewRetryClient(wrapped, b.retries, rand.New(rand.NewSource(time.Now().UnixNano())), time.Sleep)
